...
function transfer_stake_to(api, local_key) {
    ...
    // console.log(`Sign publicKey ${alice.publicKey}`);// same as who.encode()

    const promise = new Promise(function(resolve, reject){
        // do something cost time

        const status = api.tx.session
        .transferStakeTo(address, id, seed, 10)
...
async function main() {
    ...
    const result = await transfer_stake_to(api, local_keys[0]);
    if (result.isInBlock){
