...
The easiest way to start a multi-node testnet is using the Makefile commands. See the [Multi-Node Network with Make Commands](#multi-node-network-with-make-commands) section above for detailed instructions.

**Alternative: Use Shell Scripts**

You can also start a testnet using `alice.sh` & `bob.sh` scripts in the `scripts/cmd/` directory. Remember to reset boot node id before starting `bob.sh`.
...
#### Multi-Node Network with Make Commands

The project provides convenient Makefile commands to start individual nodes in a multi-node testnet:
...
2. Generate the genesis configuration:
```bash
make genesis
```
...
1. **Start Alice (Bootnode)**:
```bash
make alice
```
- Port: `30333` (P2P), `9944` (WebSocket)
- Node key: `build/keys/alice_node_key`
- Data directory: `build/tmp/alice`
- Alice acts as the bootnode for other nodes to connect to

2. **Start Bob** (in a new terminal):
```bash
make bob
```
- Port: `30334` (P2P), `9945` (WebSocket)
- Node key: `build/keys/bob_node_key`
- Data directory: `build/tmp/bob`
- Connects to Alice via bootnodes

3. **Start Charlie** (in a new terminal):
```bash
make charlie
```
- Port: `30335` (P2P), `9946` (WebSocket)
- Node key: `build/keys/charlie_node_key`
- Data directory: `build/tmp/charlie`
- Connects to Alice via bootnodes

**Node Keys Location:**

All node keys are stored in the `build/keys/` directory:
- `build/keys/alice_node_key` - Alice's Ed25519 secret key (64 hex characters)
- `build/keys/bob_node_key` - Bob's Ed25519 secret key (64 hex characters)
- `build/keys/charlie_node_key` - Charlie's Ed25519 secret key (64 hex characters)

These keys are used for libp2p networking and determine each node's peer ID. The keys are loaded via the `--node-key-file` parameter.

**Important Notes:**

- Each `make` command will **remove and recreate** the node's data directory before starting, ensuring a clean state
- Start Alice first, as Bob and Charlie depend on Alice's peer ID for connection
- All nodes use the same `genesis.json` file generated by `make genesis`
- Nodes communicate using host network mode for local testing
- mDNS discovery is enabled by default for local network peer discovery
...
Or stop individual nodes using Ctrl+C in their respective terminals.
